...
def create():
    ...
    do('''CREATE TABLE IF NOT EXISTS user (
            ...
            name VARCHAR,login VARCHAR, password VARCHAR, subscription_mode VARCHAR, subscription_date VARCHAR, is_blocked bit)''' 
    )
    close()
...
class User():
    ...
    def insert(self):
        ...
        VALUES ({self.name},{self.login},{self.password},{self.subscription_mode},{self.subscription_date},{self.is_blocked})"""
        print(query)
        cursor.execute(query)
        conn.commit()        
        close()
    ...
    def update(self):
        ...
        is_blocked={self.is_blocked} WHERE login={self.login})"""
        print(query)
        cursor.execute(query)
        conn.commit()        
        close()
